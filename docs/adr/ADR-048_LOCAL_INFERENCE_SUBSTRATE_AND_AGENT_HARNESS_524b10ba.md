...
## 5. Emenda Formal Ratificada pela Mesa Redonda (CASE-2026-08-19-DUAL-ENGINE-HARNESS-AMENDMENT)

Em deliberação tripartite com os três assentos de fronteira (**Google Gemini 3.7**, **Anthropic Fable 5** e **OpenAI GPT Sol 5.6**), fica formalmente ratificada a **Emenda Dual-Engine no Plano de Computação (`tare.tools.kernel/compute_plane`)**:

### 5.1 Invariantes Canônicos Ratificados
1. **Padrão Dual-Engine Híbrido:**
   * **`AiderDriver`**: Engine especializada em refatoração multi-arquivo de grande escala, navegação de AST com **RepoMap (Tree-Sitter + PageRank)** e integração Git-First com commits rastreáveis.
   * **`LittleCoderDriver`**: Engine especializada em **Compound Tools** e loop de auto-reparo TDD in-memory, otimizada especificamente para modelos locais (7B–35B).
2. **Capacidade Unitária de Geração (`GenerationToken`):**
   * Cada passo autorizado emite um token de capacidade de uso único e não reutilizável, impedindo loops ocultos ou descontrolados dentro dos drivers.
3. **Teto Rígido de Orçamento Linearizado ($1 \le N \le 5$):**
   * O `ExecutionBudget` e o `ExecutionEnvelope` rejeitam qualquer valor fora de $[1, 5]$. Ao atingir 5 tentativas, o failover determinístico para o Code Auditor em nuvem é disparado imediatamente.
4. **Isolamento Hermético Real de SO (`HermeticExecutor`):**
   * **Filesystem:** Jail determinístico via resolução estrita de caminhos (`Path.resolve()`).
   * **Rede:** `unshare -n` no Linux e política Fail-Closed em plataformas sem suporte verificado a namespaces.
   * **Árvore de Processos:** Encerramento completo via `killpg` (POSIX) e `taskkill /F /T` (Windows) em caso de timeout.
5. **Cockpit Operacional Local (`local_chat.py`) & Guia do Operador:**
   * Interface de terminal streaming SSE 100% offline para o operador interagir com os modelos na GPU do nó `aaaaa` com telemetria ao vivo e troca dinâmica de modelos (`/switch`).
   * Guia operacional detalhado documentado em [`docs/OPERATOR_GUIDE_LOCAL_HARNESS.md`](file:///C:/projects/tare.tools.os/docs/OPERATOR_GUIDE_LOCAL_HARNESS.md).
6. **Ergonomia Operacional & Opt-Out via Flags (Paridade com AGY / Claude Code / Codex):**
   * O isolamento hermético rigoroso e o bloqueio de rede são ativados como **Safe by Default** para automações em CI/CD não-supervisionadas (`--strict-isolation`, `--no-net`).
   * Para desenvolvimento interativo e pair programming local, o operador tem soberania total via flags explícitas de opt-out (`--no-isolation`, `--dangerously-skip-sandbox`, `--enable-net`), eliminando atritos burocráticos artificiais e permitindo acesso direto ao workspace e à rede local.
