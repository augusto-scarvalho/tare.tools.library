...
class TranslationRegistrationTests(unittest.TestCase):
 def test_registers_derivative_with_source_hash(self):
  with tempfile.TemporaryDirectory() as td:
   root=Path(td)/'research'; source=root/'corpus'/'original'/'b'/'study.md'; source.parent.mkdir(parents=True); source.write_text('# PT\ntexto\n',encoding='utf-8')
   mans=root/'corpus'/'manifests'; mans.mkdir(parents=True)
   ...
   mps=list((root/'corpus'/'manifests'/'translations'/'en').glob('*.json'));self.assertEqual(len(mps),1)
